...
import me.ranol.scriptingspells.api.CooldownManager;
...
public class CastCommand implements TabExecutor {
	...
	public List<String> onTabComplete(CommandSender s, Command c, String l, String[] a) {
		...
		if (a.length <= 1) {
			if (s.isOp()) {
				def.addAll(TabCompletor.complete(a[0], "reload", "rconf", "rspell", "edocs", "eeff", "resetcd"));
			}
	...
	public boolean onCommand(CommandSender s, Command c, String l, String[] a) {
		try {
			...
			if (a[0].equals("reload")) {
				...
				return true;
			} else if (a[0].equals("resetcd")) {
				if (s instanceof LivingEntity) CooldownManager.reset((LivingEntity) s);
				return true;
			} else if (a.length == 1) {
				if (s instanceof Player) {
